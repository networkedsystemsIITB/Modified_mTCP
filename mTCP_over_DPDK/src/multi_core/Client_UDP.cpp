...
void clientThreadFunc(void* arg1){
	...
	int n = mudp_sendto(mctx,sockfd, data, 1400, 0,
		...
		*/
		...
		for(int i=0;i<nevents;i++) {
			...
			if (events[i].events & MTCP_EPOLLOUT) {
				...
				while(1)
				{
					n = mudp_sendto(mctx,sockfd, data, DATA_LEN, 0, (struct sockaddr *) &serveraddr, serverlen);
...
int main(int argc, char **argv){
	...
	for(int i=0;i<MAX_THREADS;i++){
		...
		/*if(i==0){
			arguments[i].portno = 5900;
			arguments[i].coreno = sym_hash_fn(sip,dip,5900+SEED,SERVER_PORT+SEED)%MAX_THREADS;
