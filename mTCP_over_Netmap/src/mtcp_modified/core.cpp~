		...
		for (rx_inf = 0; rx_inf < CONFIG.eths_num; rx_inf++) {
			...
			for(i=1;i<=recv_cnt;i++){
				...
				if (pktbuf != NULL)
				{
					...
					ret = ProcessPacket(mtcp, rx_inf, ts, pktbuf, len);
					//if(mtcp->ctx->cpu != 1 && mtcp->ctx->cpu != 3)
					//cout << "Received packet" << ret << " " << mtcp->ctx->cpu << endl;
...
#define	IS_HUGEPAGE 0
	...
	for (i = 0; i < CONFIG.max_concurrency; i++) {
		...
		INIT_LIST_HEAD(&mtcp->smap[i].recv_queue);
	...
	/*UDP Variables Initialization*/
	mtcp->udp_id = 0;
	INIT_LIST_HEAD(&mtcp->udp_send_list);

	mtcp->udp_send_entries = (struct mudp_send_list *)calloc(CONFIG.udp_list_entries, sizeof(struct mudp_send_list));
	if (!mtcp->udp_send_entries) {
		perror("calloc");
		CTRACE_ERROR("Failed to allocate memory for udp_send_entries.\n");
		return NULL;
	}
	TAILQ_INIT(&mtcp->udp_free_send_list);
	for (i = 0; i < CONFIG.udp_list_entries; i++) {
		INIT_LIST_HEAD(&mtcp->udp_send_entries[i].list_member);
		TAILQ_INSERT_TAIL(&mtcp->udp_free_send_list, &mtcp->udp_send_entries[i], free_send_list_link);
	}

	mtcp->udp_receive_entries = (struct udp_receive_queue *)calloc(CONFIG.udp_list_entries, sizeof(struct udp_receive_queue));
	if (!mtcp->udp_receive_entries) {
		perror("calloc");
		CTRACE_ERROR("Failed to allocate memory for udp_receive_entries.\n");
		return NULL;
	}
	TAILQ_INIT(&mtcp->udp_free_receive_list);
	for (i = 0; i < CONFIG.udp_list_entries; i++) {
		INIT_LIST_HEAD(&mtcp->udp_receive_entries[i].recv_queue);
		TAILQ_INSERT_TAIL(&mtcp->udp_free_receive_list, &mtcp->udp_receive_entries[i], free_receive_list_link);
	}

	mtcp->udp_send_buffer = (struct mudp_send_buffer *) calloc(1,sizeof(struct mudp_send_buffer));
	mtcp->udp_receive_buffer = (struct mudp_receive_buffer *) calloc(1,sizeof(struct mudp_receive_buffer));
	init_send_buffer(mtcp->udp_send_buffer,CONFIG.udp_sndbuf_size);
	init_receive_buffer(mtcp->udp_receive_buffer,CONFIG.udp_rcvbuf_size);

	struct four_tuple_key empty_key = {0,0};
	struct four_tuple_key delete_key = {0,1};
	mtcp->mudp_flow_table.set_empty_key(empty_key);
	mtcp->mudp_flow_table.set_deleted_key(delete_key);
	//mtcp->mudp_flow_table.rehash(CONFIG.max_concurrency);
...
MTCPRunThread(void *arg)
{
	...
	if (pthread_mutex_init(&ctx->mudp_socket_lock, NULL)) {
		perror("pthread_mutex_init of ctx->mudp_socket_lock\n");
		exit(-1);
	}
