...
void handle_ds_reply1(int conn_id, void* request, void* packet, int err_code){
...
void handle_c_reply1(int conn_id, void* request, char* packet, int err_code){
        int a=5,i,key_id,id;
        mme_state *x = static_cast<mme_state*>(request); 
...
void handle_ue(int conn_id, void* request, char* packet, int err_code){
		
		int a=conn_id,i, key_id, c_id,id,vconn_id;
		id = conn_id/10000000;
		vconn_id = conn_id%(10000000);
		request = allocReqObj(conn_id, 1);
		c_id = createClient(conn_id, "169.254.9.33", "169.254.9.78", 6000, "tcp");  
		//20000000
        ...
        char* to_send = "pq";
		setData(conn_id, key_id, LOCAL, (void*)to_send,3); 
		char* pkt = getPktBuf(conn_id);
		memcpy((void*)pkt, (void*)(x->b), 3);
		sendData(c_id, pkt, 3);		
}
