...
void serverThreadFunc(void* arg1){
	...
	//initialsze client pool to connect as client
	static in_addr_t daddr;
	static in_port_t dport;
	static in_addr_t saddr_c;
	daddr = inet_addr("169.254.9.78");
    dport = htons(6000);
    saddr_c = INADDR_ANY; 
    mtcp_init_rss(mctx, saddr_c, IP_RANGE, daddr, dport);
			...
			if(     (events[i].events & MTCP_EPOLLERR) ||
            	...
            	if(events[i].data.sockid == p_core[id].ds_sockid || events[i].data.sockid == p_core[id].ds_sockid1){
            		//reply from data store
                        ...
                        fn_ptr(vnf_connid, p_core[id].mem_ptr[buf_sockid], buf_value.c_str(),0); //callback function initialised
                        ...
                        fn_ptr(vnf_connid, p_core[id].mem_ptr[events[i].data.sockid], data,0); //callback function called
...
*/
void registerCallback(int vnf_connid, enum event_type t2, void callbackFnPtr(int, void*, char*, int))
{
	int id, connID;
	id = vnf_connid/10000000;
    if(vnf_connid!= 1){
...
*/
void setData(int vnf_connid, int key, enum data_location localRemote, void* value, int value_len){
...
*/
void getData(int vnf_connid, int key, enum data_location localRemote, void callbackFnPtr(int, void*, void*, int)){
	int id, connID,ds_conn_id;
	id = vnf_connid/10000000;
    connID = vnf_connid%(10000000);
	if(localRemote==CHECKCACHE){
		...
		if(cache_check==1){
			...
			fn fn_ptr;
        	         ...
        	         fn_ptr(vnf_connid, p_core[id].mem_ptr[connID], ds_value,0);
		}
		else{
		...
		ds_lock.unlock();
		callbackFnPtr(vnf_connid, p_core[id].mem_ptr[connID], ds_value, 0);
...
*/
void delData(int vnf_connid, int key, enum data_location localRemote){
	int id, connID;
	id = vnf_connid/10000000;
    connID = vnf_connid%(10000000);
	if(localRemote==REMOTE){
